 ...
 * Respects classification filter — only sends for allowed classifications.
 *
 * @param {object} lead  — lead data { name, phone, email, practice, niche, volume, qualification, source }
 * @param {object} [opts]
 * @param {string} [opts.recipient]  — email recipient override
 * @param {string} [opts.timestamp]  — ISO timestamp override (for testing)
 * @param {string[]} [opts.onClassification]  — only send if lead.classification is in this list (default: ['hot', 'warm', 'qualified'])
 * @returns {Promise<object|null>} — Resend API response or null on failure/skip
 */
export async function notifyLead(lead, opts = {}) {
  ...
  const classification = (lead.qualification?.classification || 'unknown').toLowerCase();
  const allowedClasses = opts.onClassification || ['hot', 'warm', 'qualified'];

  // Classification filter: skip cold/unqualified leads
  if (!allowedClasses.includes(classification)) {
    console.log(`[lead-notify] Skipping email for classification "${classification}" (allowed: ${allowedClasses.join(', ')})`);
    return null;
  }
  ...
  try {
    ...
    console.log(`[lead-notify] Email sent: id=${data.id} to=${recipient} subject="${subject}"`);
    return data;
  } catch (err) {
